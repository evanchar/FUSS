...
class DinoFeaturizer(nn.Module):
    ...
    def __init__(self, dim, cfg):
        ...
        self.clip_original_centroids = load_clip_centroids()
        ...
        if self.proj_type == "nonlinear":
            ...
            # self.cluster2 = self.identity_featurizer(self.n_feats)
    ...
    def make_nonlinear_clusterer(self, in_channels):
        temp =  torch.nn.Conv2d(in_channels, in_channels, (1, 1))
        return torch.nn.Sequential(
            torch.nn.Conv2d(in_channels, in_channels, (1, 1)),
            torch.nn.ReLU(),
            torch.nn.Conv2d(in_channels, self.dim, (1, 1)))

    def identity_featurizer(self, in_channels): # TODO: this needs fixing to not train STEGO head but work with dino features
        return in_channels

    def project_clip_centroids(self, clip_centroids):
        projected_centroids = self.cluster2(clip_centroids.cpu())
        return projected_centroids.squeeze().reshape((27, 70))
...
class ClusterLookupClip(nn.Module):

    def __init__(self, dim: int, n_classes: int, clip_centroids):
        super(ClusterLookupClip, self).__init__()
        self.n_classes = n_classes
        self.dim = dim
        self.clusters = nn.Parameter(clip_centroids)

    def reset_parameters(self):
        # todo : this is the old one that randomizes clusters. Needs to change.
        with torch.no_grad():
            self.clusters.copy_(torch.randn(self.n_classes, self.dim))

    def forward(self, x, alpha, log_probs=False):
        normed_clusters = F.normalize(self.clusters, dim=1) # todo do I need to normalize the clusters??
        normed_features = F.normalize(x, dim=1)
        inner_products = torch.einsum("bchw,nc->bnhw", normed_features, normed_clusters)
...
#         self.dim = dim
#         self.clusters = nn.Parameter(clip_centroids)
#         self.in_channels = clip_centroids.shape[1]
#         # self.projector = torch.nn.Sequential(
#         #     torch.nn.Conv1d(self.in_channels, self.in_channels, kernel_size=1),
#         #     torch.nn.ReLU(),
#         #     torch.nn.Conv1d(self.in_channels, self.dim, kernel_size=1))
#
#     def reset_parameters(self):
#         # todo : this is the old one that randomizes clusters. Needs to change.
#         with torch.no_grad():
...
#         #     normed_clusters = self.projector(self.clusters.permute(1,0)).permute(1,0)
#         inner_products = torch.einsum("bchw,nc->bnhw", normed_features, normed_clusters)
#
#         if alpha is None:
#             cluster_probs = F.one_hot(torch.argmax(inner_products, dim=1), self.clusters.shape[0]) \
...
def load_clip_centroids(path=None):
    '''
    Build this into cfg to not have issues with choosing centroids
    '''
    if path is None:
        # return torch.load("./clip_text_centroids", weights_only=True).reshape((1, 768, 27, 1))
        # return torch.load("/media/FastData/evangelos/FUSS/src/clip_textSimar_centroids", weights_only=True).reshape((1, 768, -1, 1))
        return torch.load("/media/FastData/evangelos/FUSS/src/clip_text_centroids", weights_only=True).reshape((1, 768, -1, 1))
    else:
        return torch.load(path, weights_only=True).reshape((1, 768, -1, 1))
...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def forward(self,
        ...
        if self.cfg.neg_samples != 0:
            ...
            neg_losses = [torch.zeros(1)]
            neg_cds = [torch.zeros(1)]
