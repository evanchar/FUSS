...
def batched_crf(pool, img_tensor, prob_tensor): # TODO this works since pool is not utilized
    outputs = [_apply_crf((img, prob)) for img, prob in zip(img_tensor.detach().cpu(), prob_tensor.detach().cpu())]
    return torch.cat([torch.from_numpy(arr).unsqueeze(0) for arr in outputs], dim=0)


@hydra.main(config_path="configs", config_name="eval_config_cityscapes.yaml")
def my_app(cfg: DictConfig) -> None:
    pytorch_data_dir = cfg.pytorch_data_dir
    # result_dir = cfg.model_paths[0][:cfg.model_paths[0].rfind('/')]
    result_dir = os.path.dirname(cfg.model_paths[0])
    os.makedirs(join(result_dir, "img"), exist_ok=True)
    os.makedirs(join(result_dir, "label"), exist_ok=True)
    os.makedirs(join(result_dir, "cluster"), exist_ok=True)

    for model_path in cfg.model_paths:
        if model_path[-1] != 't' and "cityscapes" in model_path :
            model = LitUnsupervisedSegmenter(27, cfg)
        ...
        if run_picie:
            picie_state = torch.load("../Federated_Unsupervised_Semantic_Segmentation/src/saved_models/picie_and_probes.pth")
        ...
        if model.cfg.dataset_name == "cocostuff27":
            ...
            n_classes = model.cfg["dir_dataset_n_classes"]
            # all_good_images = range(80)
            # all_good_images = [ 5, 20, 56]
            all_good_images = [11, 32, 43, 52]
        else:
        ...
        with Pool(cfg.num_workers + 5) as pool:
            for i, batch in enumerate(tqdm(test_loader)):
                with torch.no_grad():
                    ...
                    # cluster_probs = model.cluster_probe(code, n_classes, log_probs=True)

                    # print(f"Run crf?: {cfg.run_crf}")
                    if cfg.run_crf:
                        # print(f"Starting CRFing...")
        ...
        json_path = os.path.join(result_dir, f"metrics.json")
        if not os.path.exists(json_path):
            with open(json_path, "w") as f:
                json.dump(tb_metrics, f, indent=4)
            print(f"✅ Saved metrics to: {json_path}")
        else:
            print(f"⚠️ File already exists: {json_path} — not overwritten.")
        ...
        for good_images in batch_list(range(len(all_good_images)), 10):
            ...
            for i, img_num in enumerate(good_images):
                plot_img = (prep_for_plot(saved_data["img"][img_num]) * 255).numpy().astype(np.uint8).squeeze()
                plot_label = (model.label_cmap[saved_data["label"][img_num]]).astype(np.uint8).squeeze()
            ...
            remove_axes(ax)
            plt.tight_layout()
            save_path = os.path.join(result_dir, f"comparative.png")
            plt.savefig(save_path, dpi=500)
            # plt.show()
            plt.clf()

        plot_cm(model.test_cluster_metrics.histogram, model.label_cmap, model.cfg)
        save_path = os.path.join(result_dir, f"conf_matrix.png")
        plt.savefig(save_path, dpi=500)
        # plt.show()
        plt.clf()
