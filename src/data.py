...
class CocoFed(Dataset):
    def __init__(self, root, image_set, transform, target_transform,
                 coarse_labels, exclude_things, client=None, subset=None):
        super(CocoFed, self).__init__()
        self.split = image_set
        ...
        client_num = 18  # hardcoded to reproduce experiments.

        if int(client[-2]) != 1:
            client_id = int(client[-1])
        else:
            client_id = int(client[-1]) + 10

        if self.subset is None:
            # self.image_list = "Coco164kFull_Stuff_Coarse.txt"
            self.image_list = f"../fuss_data/cocostuff/partitioned_splits/train_MA{client_num}__client{client_id - 1}.txt"
            self.image_list = os.path.normpath(self.image_list)
...
class CityscapesSeg(Dataset):
    def __init__(self, root, image_set, transform, target_transform):
        ...
        print('our_image_set: ', our_image_set)
...
class SimarSeg(Dataset):
    def __init__(self, root, image_set, transform, target_transform):
        super(SimarSeg, self).__init__()
        self.split = image_set
        self.root = join(root, "simar")
        if image_set == "train":
            # our_image_set = "train_extra"
            # mode = "coarse"
            our_image_set = "train"
...
class CroppedDataset(Dataset):
    def __init__(self, root, dataset_name, crop_type, crop_ratio, image_set, transform, target_transform):
        ...
        self.root = os.path.normpath(self.root)  # normalizes / and \ to OS format

        self.img_dir = join(self.root, "img", self.split)
        # print("Resolved img_dir:", os.path.abspath(self.img_dir))
...
class ContrastiveSegDataset(Dataset):
    def __init__(self,
        ...
        # TODO commented out to be safe that nothing is looking at cityscapes folders
        ...
        # todo : new code for correct simar MA loading. Experimental
        elif 'simar' in dataset_name or 'voc' in dataset_name and crop_type is None:
            self.n_classes = cfg.dir_dataset_n_classes
            dataset_class = DirectoryDataset
            # extra_args = dict(path=cfg.dir_dataset_name)
            extra_args = dict(path=dataset_name)
        # todo : new code for correct cityscapes MA loading. Experimental
        elif 'MA' in dataset_name and crop_type is not None:
            self.n_classes = 27
            dataset_class = CroppedDataset
            extra_args = dict(dataset_name=f"{dataset_name}", crop_type=crop_type, crop_ratio=cfg.crop_ratio)
        elif 'MA' in dataset_name and 'cocostuf' in dataset_name and crop_type is None:  # todo : federated subsampled cocostuf
            self.n_classes = 27
            dataset_class = CocoFed
            extra_args = dict(client=dataset_name, coarse_labels=False, subset=None, exclude_things=False)
        ...
        feature_cache_file = os.path.normpath(feature_cache_file)  # normalizes / and \ to OS format
        # print("Resolved feature_cache_file:", os.path.abspath(feature_cache_file))
