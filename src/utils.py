...
def cluster_analysis(clients, agg_num):
    client_num = len(clients)
    steps = int(len(clients[0]))
    steps_per_agg = int(steps // (agg_num + 1))

    dists = np.zeros(client_num)
    for i in range(client_num):
        temp = 0
        for k in range(agg_num*steps_per_agg, steps - 1):
            temp += torch.dist(clients[i][k], clients[i][(k + 1)])
        dists[i] = temp
    return dists
...
def weight_calculation(trainers):
    datapoints = []
    for i in range(len(trainers)):
        datapoints.append(trainers[i].train_dataloader().dataset)
