...
def get_class_labels(dataset_name):
    if dataset_name.startswith("cityscapes"):
        ...
        # todo: old list has double classes. Since we are reproducing the results we remove duplicates
        return [
        ...
        #     "electronic", "appliance", "food", "furniture", "indoor",
        #     "kitchen", "accessory", "animal", "outdoor", "person",
        #     "sports", "vehicle", "ceiling", "floor",
        #     "rawmaterial", "textile", "wall", "window",
        #     "building", "ground", "plant", "sky", "solid",
        #     "structural", "water"]
    ...
    # todo: this is for the simar project
    # elif dataset_name == "directory":
    #     return [
    #         'background',
    #         'pipes']

    # todo: this is for voc
    elif dataset_name == "directory":
        return [
            'background',
            'aeroplane', 'bicycle', 'bird', 'boat', 'bottle',
...
class LitUnsupervisedSegmenter(pl.LightningModule):
    def __init__(self, n_classes, cfg):
        ...
        # self.train_cluster_probe = ClusterLookupClip(dim, n_classes)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            # todo: bellow can be deleted since I append these to a list of dicts above
            return {
                'img': img[:self.cfg.n_images].detach().cpu(),
                'linear_preds': linear_preds[:self.cfg.n_images].detach().cpu(),
                "cluster_preds": cluster_preds[:self.cfg.n_images].detach().cpu(),
    ...
    def on_validation_epoch_end(self) -> None:
        with torch.no_grad():
            ...
            #     #output_num = 0
            #     output_num = random.randint(0, len(self.outputs) -1)
            #     output = {k: v.detach().cpu() for k, v in self.outputs[output_num].items()}
            #
...
@hydra.main(config_path="configs", config_name="train_config_cityscapes.yaml")
# @hydra.main(config_path="configs", config_name="train_config_simar.yaml")
# @hydra.main(config_path="configs", config_name="train_config_voc.yaml")
def my_app(cfg: DictConfig) -> None:
    ...
    # todo: replaced this with bellow gpu_args.
    # if cfg.submitting_to_aml:
    #     gpu_args = dict(gpus=1, terval=250)
    #
    ...
    gpu_args = {
        "accelerator": "gpu",
        "devices": 1,  # Or a list of GPU ids, e.g., [0, 1, 2] for all GPUs. Trains with device = 1 right now in 3 mins per epoch
    ...
    trainer = Trainer(
        ...
        callbacks=[
            ModelCheckpoint(
                ...
                every_n_train_steps=250, # prev 180
                save_top_k=2, # prev -1
                monitor="test/cluster/mIoU", # prev commented out
                mode="max", # prev commented out
            )
        ],
